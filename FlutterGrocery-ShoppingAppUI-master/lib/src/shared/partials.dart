...
Widget foodItemService(Service food,
    {double imgWidth, onLike, onTapped, bool isProductPage = false}) {
    ...
    child: Stack(
      children: <Widget>[
        ...
        Positioned(
          ...
          child: (!isProductPage)
              ? Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text(food.vName, style: foodNameText),
                    Text(food.mPriceSale.toString(), style: priceText),
                  ],
                )
              : Text(' '),
        ),
        Positioned(
            top: 10,
            left: 10,
            child: (food.mPriceSale.toString() != null)
                ? Container(
                    padding:
                        EdgeInsets.only(top: 5, left: 10, right: 10, bottom: 5),
                    decoration: BoxDecoration(
                        color: Colors.grey[600],
                        borderRadius: BorderRadius.circular(50)),
                    child: Text('-' + food.mPriceSale.toString() + '%',
                        style: TextStyle(
                            color: Colors.white, fontWeight: FontWeight.w700)),
                  )
                : SizedBox(width: 0))
